...
const getFibonacciValue = (index: number): number => {
  ...
  // Defensive: Ensure the key exists in FIBONACCI and is a valid property
  const key = fibKeys[safeIndex];
  const value = FIBONACCI[key as keyof typeof FIBONACCI];
  if (typeof value !== 'number') {
    throw new Error(
      `Invalid Fibonacci index: ${index}. No value found for key: ${key}.`
    );
  }
  return value;
};
...
export const fibonacciDuration = (index: number): number => {
  // Get Fibonacci value at the specified index
  const fibValue = getFibonacciValue(index);
  
  // Convert to a reasonable animation duration
  // Dividing by 100 gives a range of 0.01s to 0.89s for indices 0-9
  return fibValue / 100;
};
...
export const fibonacciStaggerDelay = (
  ...
  // Get the Fibonacci value for this index
  const fibValue = getFibonacciValue(index);
  
  // Calculate delay using Fibonacci number * stagger factor
  return baseDelay + (fibValue * staggerFactor);
};
...
export const monitorAnimationPerformance = <T>(
  ...
  const { 
    name, 
    threshold = getFibonacciValue(7), // 13ms (close to 16ms frame budget)
    verbose = false 
  } = options;
